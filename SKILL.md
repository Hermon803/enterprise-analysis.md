...
5. **核心产品展示**：报告底部（数据源上方）增设"核心产品"展示区，以表格形式展示（三列：产品名称 | 发布时间 | 产品介绍），至少4个产品。"发布时间"精确到月份（如"2024年11月"）。纯互联网公司展示核心App或服务，同样≥4个。这是视觉/版面元素，不属于分析维度，不违反第1条的维度锁定。
...
PDF底部必须包含 **核心产品表格**（至少4个产品，三列：产品名称 | 发布时间 | 产品介绍），用于替代原有的产品图展示区。发布时间精确到月份。
...
- 去掉所有`---`分隔线，板块间用自然间距分隔
- **核心产品展示区**：标题"核心产品"12pt，表格形式，三列（产品名称 | 发布时间 | 产品介绍），至少4行。产品名称加粗（占宽约25%），发布时间居中且用次色字号8.5pt（占宽约18%），产品介绍简要说明（一行以内）。表头深色背景白色文字。CSS参考：
  ```css
