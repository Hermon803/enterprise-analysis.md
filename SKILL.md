...
5. **核心产品展示**：报告底部（数据源上方）增设"核心产品"展示区，以表格形式展示（三列：产品名称 | 发布时间 | 产品介绍），至少4个产品。"发布时间"精确到月份（如"2024年11月"）。纯互联网公司展示核心App或服务，同样≥4个。**按发布时间降序排列**（最新产品在前）。**产品数据必须是搜索核实后的最新发布信息**，不可仅依赖已有知识库中的旧版本产品。这是视觉/版面元素，不属于分析维度，不违反第1条的维度锁定。
...
PDF底部必须包含 **核心产品表格**（至少4个产品，三列：产品名称 | 发布时间 | 产品介绍），用于替代原有的产品图展示区。发布时间精确到月份。**按发布时间降序排列**（最新产品在前）。**产品数据必须通过搜索核实**，以最新发布版本为准。
