...
5. **核心产品展示**：报告底部（数据源上方）增设"核心产品"展示区，以表格形式展示（三列：产品名称 | 发布时间 | 产品介绍），至少4个产品。"发布时间"精确到月份（如"2024年11月"）。纯互联网公司展示核心App或服务，同样≥4个。**按发布时间降序排列**（最新产品在前）。**产品数据获取流程**：
   a) 先访问公司**官网产品页**获取在售最新产品列表（如 consumer.huawei.com/cn/phones/、nvidia.com）
   b) 从产品页提取发布时间信息（常见位置：HTML meta description、URL路径中的日期、`<time>`标签、页面底部的版权年份等）
   c) 如官网无明确发布时间，搜索科技媒体发布记录（"公司名+产品名+发布"）
   d) 如网络搜索不可达，使用已有知识但**标注"发布时间为估计值"**
   e) 按发布时间降序排列（最新产品在前）

   这是视觉/版面元素，不属于分析维度，不违反第1条的维度锁定。
...
9. **Review层**：全部内容生成后，必须在HTML中以注释`<!-- ... -->`形式追加Review层，逐板块审查：①数据来源（具体到年报/机构/网页）②客观性检查（确认无主观表述）③编造检查（关键财务数据是否与原始来源交叉验证）。该注释不渲染到PDF中，仅用于内部追溯。
10. **免责声明**：每个PDF报告底部、数据来源下方，必须添加：`以上内容均为 AI 生产，仅供参考`，并标注使用的AI模型（deepseek-v4-flash）和Agent工具（Hermes Agent）。
...
| **核心产品** | 访问公司官网产品页获取在售最新产品列表，提取至少4个不同产品线的产品名称、发布月份、一句话介绍。按发布时间降序。详见 `references/core-products-table.md` |

要求每个子任务输出结构化摘要，包含数据来源。

**核心人物调研要点**：百度百科 > 官网管理团队页 > 上市公司公告(年报董监高履历) > 行业媒体。注意区分"法定代表人"（工商登记）与"实际控制人/创始人"——两者常非同一人，核心人物板块只写创始人/董事长。搜索模板：`{公司名} 创始人 百度百科` / `{创始人姓名} 籍贯 毕业 履历`。上市公司年报必须披露董监高详细履历。
...
<div class="meta"><span class="region">美国/杭州/深圳市南山区</span><span class="sep">/</span><span class="tag">信创龙头</span><span class="tag">A股上市</span><span class="tag">鸿蒙生态</span></div>
<!-- 地区+标签flex行。地区规则：国外写国家，国内写省+市，深圳写深圳市+区 -->
...
> 核心人物数据来源优先级：百度百科 > 官网管理团队页 > 上市公司公告/招股书（法披信息）。注意"工商登记的法定代表人≠创始人/实际控制人"——法定代表人可能是挂名高管，实际控制人才是创始人。多源交叉验证，取多数来源一致的版本。详见 `references/financial-indicator-template.md` 中的数据溯源方法。
...
PDF底部必须包含 **核心产品表格**（至少4个产品，三列：产品名称 | 发布时间 | 产品介绍），用于替代原有的产品图展示区。发布时间精确到月份。**按发布时间降序排列**（最新产品在前）。**产品数据应从官网产品页获取**（见第5条规则的产品数据获取流程），网络不可达时标注"发布时间为估计值"。

关键坑：`<img>` 的 `src` 属性必须用**单引号** `src='data:...'`，不能用双引号。详见陷阱6。

**SVG Logo 支持：** `find_image()` 支持 `.svg` 扩展名。SVG 嵌入 HTML 时 MIME 类型为 `data:image/svg+xml;base64,...`。部分复杂 SVG（含滤镜/渐变/外部引用）在 WeasyPrint 中可能渲染异常，此时回退到 PNG（从 SimpleIcons 抓取或百度百科）。`<img>` 的 CSS `height` 控制 SVG 尺寸，`width: auto` 自动保持宽高比。
...
可选参考模板（如存在）：
```
pandoc [公司名]_企业分析.md -o [公司名]_企业分析.docx --reference-doc=./templates/enterprise-report.docx
```

> ⚠️ 参考模板文件并非必须。如果指定了 `--reference-doc` 路径但文件不存在，`pandoc` 会报错退出。不要假设模板存在 — 先检查文件是否存在，不存在则省略该参数。
...
| 核心产品表头 | 9pt | Bold | 深色表头(#e2e8f0) + 加粗产品名，描述列8.5pt #475569 |
| 数据源 | 7pt | Regular | 浅灰色 #64748B |
...
- blockquote 使用 `text-align: justify` 两端对齐，line-height ≥ 1.85。**去掉背景填充色**，仅保留左边框（3px品牌色），视觉更清爽：
  ...
  ```
- **注释型blockquote**（如④经济数据后的注释）：缩小字号至8.5pt、灰色 #64748B，去掉左边框，以区别于正文blockquote。使用直接内联样式实现而非额外类名（减少HTML复杂度）：
  ```css
...
- 去掉所有`---`分隔线，板块间用自然间距分隔
- **核心产品展示区**：标题"核心产品"12pt，表格形式，三列（产品名称 | 发布时间 | 产品介绍），**按发布时间降序排列**，至少4行。产品名称加粗（占宽约25%），发布时间居中且用次色字号8.5pt（占宽约18%），产品介绍简要说明（一行以内）。表头深色背景白色文字。CSS参考：
  ```css
  table.products { width: 100%; font-size: 9pt; border-collapse: collapse; }
  table.products th { background: var(--brand-dark); color: white; padding: 3px 8px; text-align: left; font-weight: normal; }
...
- `references/financial-indicator-template.md` — 固定10项财务指标模板 + 3年数据规范 + 数据来源分级（含A股akshare、港股/美股yfinance、SEC EDGAR用法）
- `references/core-products-table.md` — 核心产品展示区三列表格模板（产品名称｜发布时间｜产品介绍）与CSS
- `references/company-registration-scraping.md` — 中国公司工商注册信息（注册资本/法代/企业类型）抓取实战
- `references/private-company-analysis.md` — 非上市/中小企业六维分析指南（数据缺失处理、官网图片抓取）
- `references/pdf-layout-checklist.md` — PDF排版检查清单（生成前逐项核对：字体路径/表格对齐/分页控制/色彩层级）
- `references/product-research-workflow.md` — 产品最新信息搜索核实工作流（curl官网 > Python解析 > Sogou兜底），附已知可达网站列表与代码模板
- `references/industry-chain-collection.md` — **跨企业产业链全景数据采集方法**（节点分解→并行搜索→合并审计→Excel输出），与单企业六维分析互补
...
24. **产品数据搜索超时处理**：在 Step 1 数据搜集阶段，使用 `delegate_task` 搜索最新产品发布信息可能因网络限制超时（实测 600s）。应对策略：设置较短的 delegate_task 超时（<120s），超时后回退到已有的产品知识 + 标注"数据需交叉验证"。**核心产品表格**必须按发布时间降序排列、且至少4个产品。对于发布时间不确定的产品，只标注年份（如"2020年"），切勿编造具体月份。

25. **文件同步后必须验证再提交**：在本地修改 SKILL.md（或其它将被推送的文件）后，若复制到 git 仓库工作目录（如 `/tmp/ea/`），应通过 `diff` 或 `wc -l` 验证目标文件与源文件完全一致，再执行 `git commit`。`cp` 命令自身可能因路径歧义、写入权限或进程竞争导致静默失败，未验证就提交会导致旧版本被推送。